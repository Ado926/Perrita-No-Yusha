const handler = {
  ppt: async (m, { conn, text, command, usedPrefix, args }) => {
  ...
  goppt: async (m, { conn, usedPrefix, command }) => {
    if (!m.isGroup) {
      return conn.reply(m.chat, 'Este comando solo puede ser usado en grupos.', m);
    }
...
handler.ppt.help = ['ppt @usuario'];
handler.ppt.tags = ['games'];
handler.ppt.command = ['ppt'];
handler.ppt.group = true;
handler.ppt.register = true;

handler.goppt.help = ['goppt'];
handler.goppt.tags = ['games'];
handler.goppt.command = ['goppt'];
handler.goppt.group = true;
handler.goppt.register = true;
